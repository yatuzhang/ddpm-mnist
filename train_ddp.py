...
def main():
    parser = argparse.ArgumentParser(description='Train DDPM on MNIST using denoising_diffusion_pytorch')
    ...
    model = Unet(
        ...
        dim_mults=(1, 2, 4),  # Dimension multipliers (reduced to avoid downsample factor issue)
    ...
    for epoch in range(args.epochs):
        ...
        # Evaluate
        test_loss = evaluate(diffusion, test_loader, device)
        
        print(f'Epoch {epoch:3d}: Train Loss = {train_loss:.4f}, Test Loss = {test_loss:.4f}')
        
        # Save samples
        if epoch % 2 == 0:
            save_samples(diffusion, device, epoch, save_dir=args.sample_dir)
    ...
    save_samples(diffusion, device, args.epochs, save_dir=args.sample_dir)
