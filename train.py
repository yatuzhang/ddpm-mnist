...
def save_samples(model, device, epoch, num_samples=16, save_dir='samples', writer=None):
    ...
    save_image_grid(samples, f'{save_dir}/samples_epoch_{epoch:03d}.png', 
                   nrow=4, title=f'Generated Samples - Epoch {epoch}')
    
    # Save individual samples
    save_individual_images(samples, f'{save_dir}/epoch_{epoch:03d}_individual')
    
    # Log to tensorboard
    if writer is not None:
        # Convert samples to grid for tensorboard
        grid = torch.cat([samples[i:i+4] for i in range(0, len(samples), 4)], dim=0)
        writer.add_images('Generated_Samples', grid, epoch)
...
def main():
    ...
    if args.experiment_name is None:
        ...
        args.experiment_name = f"ddpm_mnist_{timestamp}"
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.save_freq == 0 or epoch == args.epochs - 1:
            samples = save_samples(model, device, epoch, save_dir=exp_sample_dir, writer=writer)
    ...
    final_samples = save_samples(model, device, args.epochs, save_dir=exp_sample_dir, writer=writer)
